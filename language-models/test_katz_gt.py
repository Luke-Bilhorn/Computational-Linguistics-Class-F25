...
model = KatzCutOffGoodTuringLanguageModel(lm_data, 5)

def test_vocab() :
    assert vocab_prob(model) == approx(1.0, .00000001)
        
def test_perplexity() :
    assert perplexity(model, test_text) == approx(552.7, .1)
    
def test_decompress() :
    decompressor = Decompressor(training_vocab)
    assert compress_decompress_accuracy(decompressor, model, test_text) > .92
