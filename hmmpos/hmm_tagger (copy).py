...
class HMMTagger :
    def __init__(self, training_tagged, vocab):
        ...
        #self.transition_probs = {t1: {t2: sum(1 for (_, a), (_, b) in zip(pairs, pairs[1:]) if a == t1 and b == t2) for t2 in tagset} for t1 in tagset}
        ...
        for i in range(len(training_tagged) - 1):
            self.transition_probs[training_tagged[i][1]][training_tagged[i+1][1]] += 1 #(1 / (len(training_tagged) - 1))
        ...
        for i in range(len(training_tagged)):
            self.emission_probs[training_tagged[i][1]][training_tagged[i][0]] += 1 #(1/len(training_tagged))
        #for list in self.emission_probs:
        #    list /= sum(list)
        #self.emission_probs = {tag: {} for tag in tagset}
        #self.emission_probs = {tag: {word: count / sum(counts.values()) for word, count in counts.items()} for tag, counts in self.emission_probs.items()}
        ...
        #self.emission_probs = {tag: {word: (count / total if total > 0 else 0) for word, count in counts.items()} for tag, counts in self.emission_probs.items() if (total := sum(counts.values())) >= 0 }  # Python 3.8+ walrus operator }
    ...
    def pos_tag(self, text):
        ...
        I = np.argmax([d[M-1][i] for i in range(N)])

        seq = ['X' for t in range(M)]
        ...
        return [(text[t], seq[t]) for t in range(M)]

        #return [(w, 'X') for w in text]